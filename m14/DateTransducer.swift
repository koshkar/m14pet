//
//  DateTransducer.swift
//  m14
//
//  Created by Кирилл Кошкарёв on 24.08.2023.
//
...
class Transducer {
    ...
    static var sortedMassWithSections = [String]()
    ...
    func sortingByDictionaryToMass() {
        ...
        let dict = Dictionary(grouping: Transducer.newModels) { (model) -> String in
            let day = calendar.component(.day, from: model.publishedAt)
            let month = calendar.component(.month, from: model.publishedAt)
            let year = calendar.component(.year, from: model.publishedAt)
            return "\(day)-\(month)-\(year)"
        }
        
        for key in dict.keys {
            Transducer.sortedMassWithSections.append(key)
        }
        Transducer.sortedMassWithSections.sort()
        print(Transducer.sortedMassWithSections)

    }
    
    init() {
        translate()
        sortingByDictionaryToMass()
    }
